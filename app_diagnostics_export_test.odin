...
test_export_writes_a_bounded_bundle_with_a_manifest :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, diagnostics_export(logs_root, session_id, destination, {}, false), 0)
...
test_export_refuses_a_destination_that_already_exists :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, diagnostics_export(logs_root, session_id, destination, {}, false), 1)
...
test_export_omits_payloads_unless_asked :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, diagnostics_export(logs_root, session_id, metadata_only, {}, false), 0)
	...
	testing.expect_value(t, diagnostics_export(logs_root, session_id, with_payloads, {}, true), 0)
...
test_export_selector_narrows_the_session_stream :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, diagnostics_export(logs_root, session_id, destination, selector, false), 0)
	session_text := export_test_text(t, destination, "session.jsonl")
	defer delete(session_text, context.allocator)
