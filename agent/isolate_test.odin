...
// suite's heaviest actors (real processes, signal handlers, loopback servers),
// so one runs at a time while the rest of the suite stays fully parallel.
// The guard is released before every fail path below, so a failed child can
...
// token, signal dispositions, or pid-keyed paths. It follows the sqlite OOM
// precedent in db/sqlite: the parent spawns the child filtered to this one
// test and checks it ran exactly once and passed; the child runs the real
// body. procedure is the caller's #procedure. True means this is the child,
// so run the body; false means the parent already checked the child, so
// return.
