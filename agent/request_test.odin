...
test_request_record_carries_the_prepared_inventory :: proc(t: ^testing.T) {
	...
	recorded, record_error := chat_request_input_json(&prep, &prep.history, chat.skill_snapshot_seq, len(prep.history.entries), Chat_Attempt{number = 1}, nil)
	if record_error != .None { testing.fail_now(t, "the request record could not be encoded") }
