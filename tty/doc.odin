...
//
// The package is named `tty` rather than `terminal` so that it can be imported
...
// (testing/reporting.odin, runner.odin, signal_handler_libc.odin).
//
...
// tests/widgets_tests, all invoked by scripts/test. `odin test ./tty` would
