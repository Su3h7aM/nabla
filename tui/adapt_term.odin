...
presentation_modifier :: proc "contextless" (modifier: Modifier) -> term.Modifier {
	switch modifier {
	case .Bold:
		return .Bold
	case .Dim:
...
presentation_modifiers :: proc "contextless" (modifiers: Modifiers) -> term.Modifiers {
	result: term.Modifiers
...
presentation_color :: proc "contextless" (color: Color) -> term.Color {
	switch value in color {
	case Default_Color:
		return term.Default_Color{}
	case Indexed_Color:
		return term.Indexed_Color(value)
	case RGB_Color:
		return term.RGB_Color(value)
	case nil:
		return nil
	}
	return nil
...
presentation_style :: proc "contextless" (style: Style) -> term.Presentation_Style {
	return {
...
// terminal half-drawn.
@(require_results)
build_frame :: proc(buffer: Cell_Buffer, cells: []term.Cell) -> (frame: term.Frame_Buffer, ok: bool) {
