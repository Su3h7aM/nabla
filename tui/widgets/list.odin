...
draw_list_rect :: proc(buffer: ^term.Frame_Buffer, rect: tui.Cell_Rect, list: List, state: ^List_State) -> (rows: int) {
...
// draw_list draws into the active layout box and keeps the selected item in
// its visible window.
draw_list_context :: proc(ctx: ^tui.Context, list: List, state: ^List_State) -> (rows: int) {
	...
	for row in 0 ..< visible {
		...
		if index == state.selected {
			tui.fill_at(ctx, line, " ", list.selected_style)
			_, _ = tui.draw_text_at(ctx, line, list.items[index], list.selected_style)
		} else {
			_, _ = tui.draw_text_at(ctx, line, list.items[index], list.style)
		}
		rows += 1
	}
	return rows
