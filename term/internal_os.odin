...
get_environment_color :: proc() -> Color_Depth {
	...
	if term_env, err := os.lookup_env(buf[:], "TERM"); err == nil {
		if strings.contains(term_env, "-truecolor") {
			return .True_Color
		}
		if strings.contains(term_env, "-256color") {
			return .Eight_Bit
		}
		if strings.contains(term_env, "-16color") {
			return .Four_Bit
		}
