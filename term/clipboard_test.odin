...
// directly, including a payload whose base64 is not the text itself and one
// carrying a newline and a byte above ASCII, which is the case a transcript
// selection always is.
@(test)
test_clipboard_sequence_frames_the_base64_payload :: proc(t: ^testing.T) {
	for original in ([]string{"hello, world", "first line\nsecond line — ok"}) {
		sequence, sequence_err := _clipboard_sequence(original, context.allocator)
		if !testing.expect_value(t, sequence_err, nil) { continue }
		...
		if !testing.expect_value(t, decode_err, nil) { continue }
