...
// checks. The mechanism mirrors agent/isolate_test.odin and is duplicated
// rather than shared because foundation packages never import the harness.
ISOLATED_CHILD_VARIABLE :: "NABLA_ISOLATED_CHILD"

// test_isolate_guard serializes isolated children: one runs at a time while
// the rest of the suite stays fully parallel.
test_isolate_guard: sync.Mutex

// test_isolate_process re-runs a process-global test in a child of this test
// binary. procedure is the caller's #procedure. True means this is the child,
