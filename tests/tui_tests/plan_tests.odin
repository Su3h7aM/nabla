...
plan_ops :: proc(
	...
	planned, required, redraw, err := tui.plan_presentation(current, previous, term.profile_default(), term.Capabilities{}, scratch)
...
plan_bytes :: proc(current: tui.Cell_Buffer, previous: Maybe(tui.Cell_Buffer), scratch: []term.Presentation_Op, bytes: []byte) -> (out: string, ok: bool) {
...
test_plan_presentation_nil_previous_forces_full_redraw :: proc(t: ^T) {
	...
	expect_value(t, ops[0], term.Presentation_Op(term.Set_Style_Op{}))
	expect_value(t, ops[1], term.Presentation_Op(term.Move_Cursor_Op{position = {x = 0, y = 0}}))
	expect_value(t, ops[2], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "a", width = 1}))
	expect_value(t, ops[3], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "b", width = 1}))
	expect_value(t, ops[4], term.Presentation_Op(term.Move_Cursor_Op{position = {x = 0, y = 1}}))
	expect_value(t, ops[5], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "c", width = 1}))
...
test_plan_presentation_changed_cell_plans_a_write :: proc(t: ^T) {
	...
	expect_value(t, ops[0], term.Presentation_Op(term.Move_Cursor_Op{position = {x = 0, y = 0}}))
	expect_value(t, ops[1], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "y", width = 1}))
...
test_plan_presentation_blank_change_plans_an_erase :: proc(t: ^T) {
	...
	expect_value(t, len(ops), 2)
	expect_value(t, ops[0], term.Presentation_Op(term.Move_Cursor_Op{position = {x = 0, y = 0}}))
	expect_value(t, ops[1], term.Presentation_Op(term.Erase_Cells_Op{count = 2}))
...
test_plan_presentation_reports_exact_required :: proc(t: ^T) {
	...
	ops, required, full_redraw, err := tui.plan_presentation(current, nil, term.profile_default(), term.Capabilities{}, scratch[:])
	...
	planned, planned_required, _, planned_err := tui.plan_presentation(current, nil, term.profile_default(), term.Capabilities{}, full_scratch[:])
...
test_plan_presentation_rejects_invalid_buffers :: proc(t: ^T) {
	...
	_, _, _, err := tui.plan_presentation({width = -1, height = 2}, nil, term.profile_default(), term.Capabilities{}, scratch[:])
	...
	_, _, _, err = tui.plan_presentation(short, nil, term.profile_default(), term.Capabilities{}, scratch[:])
	expect_value(t, err, tui.Presentation_Error.Invalid_Buffer)
	...
	_, _, _, err = tui.plan_presentation(current, Maybe(tui.Cell_Buffer)(bad_previous), term.profile_default(), term.Capabilities{}, scratch[:])
	expect_value(t, err, tui.Presentation_Error.Invalid_Buffer)
}
...
test_plan_presentation_reserves_the_corner :: proc(t: ^T) {
	...
	expect_value(t, corner_ops[0], term.Presentation_Op(term.Set_Style_Op{}))
	expect_value(t, corner_ops[2], term.Presentation_Op(term.Erase_Cells_Op{count = 2}))
}
...
test_plan_presentation_coalesces_style_runs :: proc(t: ^T) {
	...
	expect_value(t, ops[0], term.Presentation_Op(term.Move_Cursor_Op{position = {x = 0, y = 0}}))
	expect_value(t, ops[1], term.Presentation_Op(term.Set_Style_Op{style = {foreground = term.Color(term.RGB_Color{255, 0, 0})}}))
	expect_value(t, ops[2], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "a", width = 1}))
	expect_value(t, ops[3], term.Presentation_Op(term.Write_Grapheme_Op{grapheme = "b", width = 1}))
}
...
test_plan_presentation_erase_carries_the_cell_style :: proc(t: ^T) {
	...
	expect_value(t, ops[1], term.Presentation_Op(term.Set_Style_Op{style = {background = term.Color(term.RGB_Color{255, 0, 0})}}))
	expect_value(t, ops[2], term.Presentation_Op(term.Erase_Cells_Op{count = 1}))
