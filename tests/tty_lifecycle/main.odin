...
// PTY lifecycle tests for the tty package.
//
// This is a single-threaded executable (run by scripts/test with
// -define:NABLA_TTY_TEST_HOOKS=true), not an in-package @(test) suite:
...
run_child :: proc(slave_path: string, sync_out_w: posix.FD, sync_in_r: posix.FD) -> int {
	...
	check(tty.close(nil) == nil, "close(nil) is a documented no-op")
	check(tty.close(session) == nil, "close must succeed")
	...
	session, open_err = tty.open({input_mode = .Raw})
	check(open_err == nil, "open must succeed")
	live := len(tracking.allocation_map)
	tty.Terminal_Test_Fail_Next_Teardown()
	first_err := tty.close(session)
	...
	vp, vp_err := tty.viewport(session)
	...
	frame := tty.Frame_Buffer {
		columns = 2,
		rows    = 1,
		cells   = []tty.Cell{{grapheme = "a", width = 1}, {grapheme = "b", width = 1}},
	}
	scratch: [1024]byte
	tty.Terminal_Test_Zero_Write_Next()
	committed, required, present_err := tty.present(session, frame, tty.profile_default(), {}, scratch[:])
	check(present_err == tty.General_Error.Partial_Write, "a zero-progress write must report Partial_Write")
	...
	recovery_committed, recovery_required, recovery_err := tty.present(session, frame, tty.profile_default(), {}, scratch[:])
	...
	session, open_err = tty.open({input_mode = .Raw})
	check(open_err == nil, "open must succeed")
	file, file_err := tty.session_file(session)
	...
	_, _, eintr_err := tty.present(session, big_frame, tty.profile_default(), {}, big_scratch)
	...
	tty.Terminal_Test_Fail_Close_Next()
	first_err = tty.close(session)
	...
	session, open_err = tty.open({})
	check(open_err == nil, "open must succeed")
	operations := []tty.Presentation_Op {
		tty.Move_Cursor_Op{{x = 0, y = 0}},
		tty.Set_Style_Op{{foreground = tty.Color(tty.RGB_Color{0, 255, 0})}},
		tty.Write_Grapheme_Op{grapheme = "ok", width = 1},
	}
	op_scratch: [4096]byte
	op_committed, op_required, op_err := tty.present_operations(session, operations, tty.profile_default(), op_scratch[:])
	...
	max_ops := []tty.Presentation_Op {
		tty.Move_Cursor_Op{{x = max(int), y = max(int)}},
		tty.Write_Grapheme_Op{grapheme = "x", width = 1},
		tty.Erase_Cells_Op{count = max(int)},
	}
	max_scratch: [4096]byte
	max_committed, max_required, max_err := tty.present_operations(session, max_ops, tty.profile_default(), max_scratch[:])
